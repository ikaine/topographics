...
public class Tracer : MonoBehaviour
{
    ...
    //Draw height vars
    int cRadius = 10;
    float height = 0.005f; // define the height of the affected verteces of the terrain. 0.005f == 5 meters when terrain height is 1000
    ...
    public void toggleHeightLine()
    {
        heightLine = (heightLine ? false : true);
    }

    public void toggleTrees()
    {
        trees = (trees ? false : true);
    }

    public void unsetToggles()
    {
        drawHeight = false;
        heightLine = false;
        trees = false;
    ...
    void Update()
    {
        if (drawHeight)
        {
            ...
            {
                objPlane = new Plane(drawCamera.transform.forward * -1, this.transform.position);
                Ray mRay = drawCamera.ScreenPointToRay(Input.mousePosition);
        ...
        if (heightLine)
        {
            ...
            if (activeLine != null)
            {
                //Vector3 mousePos = drawCamera.ScreenToWorldPoint(Input.mousePosition);
        ...
        if (trees)
        {
            if (((Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Moved) || Input.GetMouseButton(0)))
            {
                ...
                if (objPlane.Raycast(mRay, out rayDistance))
                {
                    ...
                    float[,] treeArea = TerrainMain.terrainData.GetHeights((xPos - cRadius), (zPos - cRadius), (2 * cRadius), (2 * cRadius));
                    for (int i = 0; i < (2 * cRadius); i++)
                    {
                        for (int j = 0; j < (2 * cRadius); j++)
                        {
                    ...
                    TerrainMain.terrainData.SetHeightsDelayLOD((xPos - cRadius), (zPos - cRadius), treeArea);
                    TerrainMain.ApplyDelayedHeightmapModification();
                }
            }
        }
